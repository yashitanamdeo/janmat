...
export const AttendancePage: React.FC = () => {
    ...
    const fetchAttendance = async () => {
        try {
            ...
            const [historyRes, todayRes] = await Promise.all([
                axios.get('https://janmat-backend-r51g.onrender.com/api/attendance/my-attendance', {
                    headers: { Authorization: `Bearer ${token}` }
                }),
                axios.get('https://janmat-backend-r51g.onrender.com/api/attendance/today', {
                    headers: { Authorization: `Bearer ${token}` }
                })
            ]);
    ...
    const handleCheckIn = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.post('https://janmat-backend-r51g.onrender.com/api/attendance/check-in', {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchAttendance();
        } catch (error: any) {
    ...
    const handleCheckOut = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.post('https://janmat-backend-r51g.onrender.com/api/attendance/check-out', {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchAttendance();
        } catch (error: any) {
