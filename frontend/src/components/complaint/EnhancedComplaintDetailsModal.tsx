...
export const EnhancedComplaintDetailsModal: React.FC<EnhancedComplaintDetailsModalProps> = ({ isOpen, onClose, complaintId, onUpdate }) => {
    ...
    const fetchComplaintDetails = async () => {
        ...
        try {
            ...
            const response = await axios.get(`https://janmat-backend.onrender.com/api/complaints/${complaintId}`, {
    ...
    const handleStatusUpdate = async (newStatus: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.patch(`https://janmat-backend.onrender.com/api/officer/complaints/${complaintId}/status`, {
                status: newStatus,
                                            ...
                                            {complaint.attachments.length === 0 ? (
                                                ...
                                                complaint.attachments.map((att) => (
                                                    <div key={att.id} className="relative group rounded-lg overflow-hidden border border-[var(--border-color)] aspect-square">
                                                        {att.type === 'IMAGE' ? (
                                                            <img src={`https://janmat-backend.onrender.com/${att.url}`} alt="Attachment" className="w-full h-full object-cover" />
                                                        ) : (
                                                        ...
                                                        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                                                            <a
                                                                href={`https://janmat-backend.onrender.com/${att.url}`}
                                                                target="_blank"
