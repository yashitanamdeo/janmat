...
export const EnhancedComplaintDetailsModal: React.FC<EnhancedComplaintDetailsModalProps> = ({ isOpen, onClose, complaintId, onUpdate }) => {
    ...
    const fetchComplaintDetails = async () => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3000/api/complaints/${complaintId}`, {
    ...
    const handleStatusUpdate = async (newStatus: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.patch(`http://localhost:3000/api/officer/complaints/${complaintId}/status`, {
                status: newStatus,
                                            ...
                                            {complaint.attachments.length === 0 ? (
                                                ...
                                                complaint.attachments.map((att) => (
                                                    <div key={att.id} className="relative group rounded-lg overflow-hidden border border-[var(--border-color)] aspect-square">
                                                        {att.type === 'IMAGE' ? (
                                                            <img src={`http://localhost:3000/${att.url}`} alt="Attachment" className="w-full h-full object-cover" />
                                                        ) : (
