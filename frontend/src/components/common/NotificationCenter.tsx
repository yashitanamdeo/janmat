...
export const NotificationCenter: React.FC = () => {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const response = await axios.get('https://janmat-backend-r51g.onrender.com/api/notifications', {
    ...
    const markAsRead = async (id: string) => {
        try {
            const token = localStorage.getItem('token');
            await axios.patch(`https://janmat-backend-r51g.onrender.com/api/notifications/${id}/read`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const markAllAsRead = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.patch('https://janmat-backend-r51g.onrender.com/api/notifications/read-all', {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
